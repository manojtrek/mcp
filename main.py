...
from config import SYSTEM_PROMPT, AZURE_OPENAI_CONFIG, BUILTIN_TOOLS
from mcp_handlers import simulate_mcp_tool_execution
from user_management import refresh_user_data
...
def main():
    ...
    st.set_page_config(
        page_title="Linear Project Assistant",
        page_icon="🚀",
        layout="wide",
    ...
    with col1:
        # Sidebar with user-specific tasks and MCP management
        with st.sidebar:
            # User profile section
            render_user_profile_section()
            
            # Tabs for different sections
            tab1, tab2, tab3 = st.tabs(["📋 My Tasks", "🛠️ MCP Servers", "⚙️ Settings"])
            
            with tab1:
                render_user_tasks_section()
    ...
    # Right sidebar - MCP server status and tools
    with st.sidebar:
        st.header("🛠️ Tools & Status")
        
        # Demo MCP Tools section
        render_demo_tools_section()
        
        # Tools and status section
        render_tools_status_section()
